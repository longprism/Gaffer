  ...
  <body>
    ...
    <nav class="c-nav">
      ...
      <a href="<?php echo base_url(); ?>"><img class="logo" src="<?php echo base_url(); ?>asset/img/logo 1.png"></a>
        <div class="nav-menu">
          <a class="c-button-direct" href="#">create review</a>
    ...
    <div class="c-container mb">
      <div class="flex-row">
        <div class="flex-col main-row w-sm-100 w-md-100 w-lg-66">
          <h1 class="l mb upcase">write review</h1>
          <form class="flex-col" method="post" action="<?php echo base_url(); ?>home/addreview">
            <input name="id-user" type="hidden" value="<?php echo $userdata[0]->id_user; ?>">
            <select name="id-movie" class="w-50 mb" id="select-state" placeholder="Pick a movie" required>
              <option></option>
              <?php foreach($movies as $mov) {?>
              <option value="<?php echo $mov->id_film ?>"><?php echo $mov->judul_film ?>, <?php echo $mov->tahun ?></option>
              <?php } ?>
            </select>
            <label>Review</label>
              <textarea class="w-75 mb" name="review" cols="20" rows="7" placeholder="Write here..."></textarea>
            <div class="flex-row">
